...
bool HFormBottomLoops::ShouldTransformLoop(HLoopInformation_X86* loop,
  ...
  if (exit_block->GetPredecessors().size() != static_cast<size_t>(1)) {
    PRINT_PASS_MESSAGE(this, "Exit block predecessor is more than 1");
    return false;
  }
...
void HFormBottomLoops::RewriteLoop(HLoopInformation_X86* loop,
  ...
  if (suspend_check != nullptr) {
    ...
    // neeraj - resolve dex2oat crash (disbaled check before moving SuspendCheck to the first block)
    suspend_check->MoveBefore(first_block->GetFirstInstruction(), /* do_checks */ false);
  }
...
void HFormBottomLoops::DoCFGTransformation(HGraph_X86* graph,
  ...
  PRINT_PASS_OSTREAM_MESSAGE(this, "New loop exit block " << split_exit_block->GetBlockId());
    //Add try catch information to newly added block
  if (loop_header->IsTryBlock()) {
    split_exit_block->SetTryCatchInformation(loop_header->GetTryCatchInformation());
  }
  ...
  //Add try catch information to newly added block
  if (loop_header->IsTryBlock()) {
    around_block->SetTryCatchInformation(loop_header->GetTryCatchInformation());
  }
  ...
  //Add try catch information to newly added block
  if (loop_header->IsTryBlock()) {
    top_block->SetTryCatchInformation(loop_header->GetTryCatchInformation());
  }
  ...
  if (loop_header->GetSuccessors().size() > 1 &&
    ...
    //Add try catch information to newly added block
    if (loop_header->IsTryBlock()) {
      new_block->SetTryCatchInformation(loop_header->GetTryCatchInformation());
    }
