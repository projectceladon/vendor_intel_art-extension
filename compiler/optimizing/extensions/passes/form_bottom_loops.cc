...
void HFormBottomLoops::Run() {
  ...
  for (HOnlyInnerLoopIterator it(graph_loop_info); !it.Done(); it.Advance()) {
    ...
    RewriteLoop(loop_info, loop_header, exit_block);
...
bool HFormBottomLoops::ShouldTransformLoop(HLoopInformation_X86* loop_info,
  ...
  HBasicBlock* first_back_edge = loop_info->GetBackEdges()[0];
...
void HFormBottomLoops::RewriteLoop(HLoopInformation_X86* loop_info,
  ...
  bool first_predecessor_is_preheader = loop_header->GetPredecessors()[0] == pre_header;
  ...
  for (HInstructionIterator phi_it(loop_header->GetPhis()); !phi_it.Done(); phi_it.Advance()) {
    ...
    FixPhiUses(loop_info, first_block, exit_block, phi, computed_in_header,
  ...
  // edges.
  const ArenaVector<HBasicBlock*>& back_edges = loop_info->GetBackEdges();
  std::map<HInstruction*, HInstruction*> cloned_instructions;
  for (HBasicBlock* back_block : back_edges) {
    ...
    DCHECK(back_block->GetSuccessors()[0] == loop_header);
  ...
  for (auto insn : header_values_used_in_exit_block) {
    ...
    HBasicBlock* new_around_to_exit =
      loop_header->GetSuccessors()[first_successor_is_exit ? 0 : 1];
    if (exit_block->GetPredecessors()[0] == new_around_to_exit) {
      new_phi->AddInput(insn);
      new_phi->AddInput(clone_it->second);
    } else {
      DCHECK(exit_block->GetPredecessors()[1] == new_around_to_exit);
      new_phi->AddInput(clone_it->second);
      new_phi->AddInput(insn);
    }
...
void HFormBottomLoops::FixPhiUses(HLoopInformation_X86* loop_info,
  ...
  for (HUseIterator<HEnvironment*> it(phi->GetEnvUses()); !it.Done(); it.Advance()) {
    ...
    HPhi* phi_to_replace_input = find_phi_to_replace(user->GetHolder()->GetBlock());
