...
void HGenerateSelects::TryGeneratingSelects(HBasicBlock* block) {
  ...
  HBasicBlock* first_pred = block->GetPredecessors()[0];
  HBasicBlock* second_pred = block->GetPredecessors()[1];

  // Is this a simple diamond from an HIf?
  if (first_pred->GetPredecessors().size() != 1u ||
      second_pred->GetPredecessors().size() != 1u ||
      NotFromSameIf(first_pred->GetPredecessors()[0],
                    second_pred->GetPredecessors()[0])) {
    return;
  }

  // Find elements of the pattern.
  HBasicBlock* if_block = first_pred->GetPredecessors()[0];
  ...
  // We should be careful with the type. If one of the input is integer
  // constant we should use other input for type because its type might
  // be more specific and right.

  Primitive::Type type = true_value->IsIntConstant() ?
                         false_value->GetType() : true_value->GetType();
  HCondition* cond = if_condition->AsCondition();
  HX86SelectValue* replacement =
    new (graph->GetArena()) HX86SelectValue(
                             type,
                             cond->GetCondition(),
                             cond->GetOppositeCondition(),
                             true_value, false_value,
                             cond->GetLeft(), cond->GetRight());
