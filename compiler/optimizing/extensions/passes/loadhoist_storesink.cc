...
bool LoadHoistStoreSink::DoLoadHoistStoreSink(HLoopInformation_X86* loop,
  ...
  for (auto load_store = get_to_set.rbegin();
    ...
    // neeraj - resolve dex2oat crash (disbaled check before moving get to the other block)
    get->MoveBefore(pre_header->GetLastInstruction(), false);
    ...
    // neeraj - resolve dex2oat crash (disbaled check before moving Set to the other block)
    set->MoveBefore(exit_block->GetFirstInstruction(), false);
