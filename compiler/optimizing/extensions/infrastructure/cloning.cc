...
HEnvironment* HInstructionCloner::CloneEnvironment(HEnvironment* env, HInstruction* clone) {
  ...
  for (HEnvironment* env_clone_prev = nullptr;
    ...
    // neeraj - resolve build error (modified "HEnvironment" constructor as per O-Master)
    env_clone = new (arena_) HEnvironment(arena_, env->Size(),
        env->GetMethod(), env->GetDexPc(), clone);
...
void HInstructionCloner::VisitArrayLength(HArrayLength* instr) {
  if (cloning_enabled_) {
    ...
    HArrayLength* clone = new (arena_) HArrayLength(input, instr->GetDexPc(),instr->IsStringLength());
    CommitClone(instr, clone);
  }
}
...
void HInstructionCloner::VisitInvokeStaticOrDirect(HInvokeStaticOrDirect* instr) {
  if (cloning_enabled_) {
    // neeraj - resolve build error (modified "HInvokeStaticOrDirect" constructor as per O-Master)
...
void HInstructionCloner::VisitLoadClass(HLoadClass* instr) {
  if (cloning_enabled_) {
    ...
    HLoadClass* clone =
        new (arena_) HLoadClass((input != nullptr)? input->AsCurrentMethod() : nullptr,
                                instr->GetTypeIndex(),
                                instr->GetDexFile(),
                                instr->GetClass(),
...
void HInstructionCloner::VisitStaticFieldGet(HStaticFieldGet* instr) {
  if (cloning_enabled_) {
    ...
    GetInputsForUnary(instr, &input);
    const FieldInfo& fi = instr->GetFieldInfo();
    // neeraj - resolve build error (modified "HStaticFieldGet" constructor as per O-Master)
...
void HInstructionCloner::VisitStaticFieldSet(HStaticFieldSet* instr) {
  if (cloning_enabled_) {
    ...
    const FieldInfo& fi = instr->GetFieldInfo();
    // neeraj - resolve build error (modified "HStaticFieldSet" constructor as per O-Master)
